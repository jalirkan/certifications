...
Nineteen cases ship today, every domain covered and weighted roughly to the blueprint: four on audit
execution and planning (D1), three on IT governance (D2), three on acquisition and implementation
(D3), five on operations and resilience (D4), four on protection of information assets (D5).
Between them they reach 52 of the 60 outline topics, and every one of the 23 decision rules governs
at least one case.
...
python run_tests.py                # all 491 tests
...
- Vary the answer key. The bank currently sits at A 26% / B 25% / C 25% / D 24% across 501 questions,
...
| Branching cases | 19, with graded options, taints and a path debrief; 52 of 60 topics reached |
...
| Decision rules | 23 documented, 390 of 501 questions mapped, all multi-domain |
...
| Tests | **483**, all passing; `validate` clean with no warnings |
...
1. **More branching cases.** Eleven, blueprint-weighted, every domain covered. The project wants
   roughly 35; the format and the standard are settled, so what remains is volume.
